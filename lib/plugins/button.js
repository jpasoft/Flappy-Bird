ig.module('plugins.button').requires('impact.entity').defines(function() {
	Button = ig.Entity.extend({
		...
		pressedUp: function() {},
		_inButton: function() {
			return ig.input.mouse.x / ig.game.curScale + ig.game.screen.x > this.pos.x && ig.input.mouse.x / ig.game.curScale + ig.game.screen.x < this.pos.x + this.size.x && ig.input.mouse.y / ig.game.curScale + ig.game.screen.y > this.pos.y && ig.input.mouse.y / ig.game.curScale + ig.game.screen.y < this.pos.y + this.size.y;
		}
	});
});﻿